# CData Dremio ARP Connector for Apache Hive

The Apache Hive connector allows Dremio to connect to and query data in Apache Hive. This can then allow you to build custom reports, dashboards, or even just ad-hoc SQL via your client tool of choice. Note that it does require a third-party JDBC driver that is not free, but does allow a free trial.
...
3. Download and install the [Apache Hive JDBC Driver from CData](https://www.cdata.com/drivers/hive/download/jdbc)* and copy the JAR file to the /jars/3rdparty/ directory of your Dremio installation. For example:
